...
function TimeField(id) {
    ...
    this.getHtml = function(objReference) {
        this.lastReference = objReference;
        var html = '<span id="' + this.id + '">';
        html += '<input type="text" onchange="' + objReference + '.startTime = this.value;" value="'+this.startTime+'" class="campos required" maxlength="8" size="5" onblur="valid_valida(this)"  /> - ';
        html += '<input type="text" onchange="' + objReference + '.endTime = this.value;" value="'+this.endTime+'" class=" campos required" maxlength="8" size="5" onblur="valid_valida(this)"  />';
        html += "</span>";
        return html;
    };
...
addAction = function(action_spec) {
    ...
    caption = caption+"<a href='#' class='remove' onclick=\"removeAction($('" + action_spec.id + "')); return false;\" style='float:right'></a>";
