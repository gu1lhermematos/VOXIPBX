...
class Snep_Locale {
    ...
    public function __construct() {
        ...
        if (file_exists(TRANSLATIONS_PATH . DIRECTORY_SEPARATOR . $language)) {
            $translate = new Zend_Translate(
                    array(
                'adapter' => 'gettext',
                'content' => TRANSLATIONS_PATH . DIRECTORY_SEPARATOR . $language,
                'scan' => Zend_Translate::LOCALE_DIRECTORY
                    )
            );
        } else if (file_exists(TRANSLATIONS_PATH . DIRECTORY_SEPARATOR . $language_alt)) {
            $translate = new Zend_Translate(
                    array(
                'adapter' => 'gettext',
                'content' => TRANSLATIONS_PATH . DIRECTORY_SEPARATOR . $language_alt,
                'scan' => Zend_Translate::LOCALE_DIRECTORY
                    )
            );
        } else {
        ...
        $zend_validate_translator = new Zend_Translate_Adapter_Array(
                TRANSLATIONS_PATH . "/Zend_Validate/$validate_locale/Zend_Validate.php", $language
        );
        Zend_Validate_Abstract::setDefaultTranslator($zend_validate_translator);
    }

    /**
     * isTimezone - Assert if a timezone identifier is valid or not.
    ...
    /**
     * getAvailableLanguages - Return all the languages available on the system.
     * @return <array> available languages
     */
    public function getAvailableLanguages() {
        if (count($this->availableLanguages) === 0) {
            foreach (scandir(TRANSLATIONS_PATH) as $filename) {
                if (!preg_match("/.*\.actions$/", $filename) && !preg_match("/.*\.Zend_Validate$/", $filename)) {
