...
class ContactsController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        ...
        $this->view->filter_value = Snep_Filter::setSelect($select, array("n`.`name", "city", "state", "cep", "phone_1", "cell_1"), $this->_request);
        $this->view->order = Snep_Order::setSelect($select, array("ide", "nome", "name", "city", "state", "cep", "phone_1", "cell_1"), $this->_request);
        ...
        $this->view->title = $this->view->translate("contacts");
    ...
    public function addAction() {
        ...
        $form->getElement('id')->setValue(Snep_Contacts_Manager::getLastId())->setAttrib('readonly', true);
    ...
    /**
     * importAction - Import contacts from CSV file
     */
    ...
    /**
     * csvAction - Associate fields between database and csv file
     */
    ...
    /**
     * csvprocessAction - Process a csv file
     */
    public function csvprocessAction() {
        if ($this->getRequest()->isPost()) {
            ...
            if (count($error)>0){
                $this->view->error = $this->view->translate('The following entries of the CSV file have null data<br/>');
            }else{
            	$this->_redirect($this->getRequest()->getControllerName());
            }
        }
    ...
    public function inuseAction() {
    	...
    	$db = Zend_Registry::get('db');
    	$select = $db->select()
    	->from(array("contacts_names"), array("id"))
    	->where("CONCAT(id) LIKE '%$id%'");
    	$query = $db->query($select);
    	$inuse = $query->fetchAll();
    	
    	$options = '';
    	if(count($inuse) > 0){
    		foreach($inuse as $cod) $options .= ", ".$cod['id'];
